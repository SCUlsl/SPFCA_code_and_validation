...
class unet(nn.Module):
    def __init__(self, in_channels, out_channels, depth, basewidth, drop_rate,pretrained=False, backbone='vgg'):
        super(unet, self).__init__()
        self.Encoder = unet_Encoder(in_channels=in_channels,depth=depth,basewidth=basewidth,drop_rate=drop_rate)
        in_channels = basewidth*2**(depth-1)
        self.Decoder = unet_Decoder(in_channels=in_channels,out_channels=out_channels,depth=depth,basewidth=basewidth,drop_rate=drop_rate)
